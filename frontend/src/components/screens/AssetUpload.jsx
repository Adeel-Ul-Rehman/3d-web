...
const AssetUpload = () => {
  ...
  const handleGenerate = () => {
    navigate('/generating', { state: { template, prompt, answers, files } });
  };
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="glass-effect rounded-3xl p-6 md:p-10 shadow-xl">
            <div className="mb-6">
              ...
              <p className="text-xs text-gray-400 mt-1">Step 3 of 3 · Optional reference photos, logos, or 3D models</p>
            </div>
            ...
            <div className="mt-8 space-y-6">
              ...
              {files.length > 0 && (
                <div className="space-y-3">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Uploaded Files ({files.length})</h3>
                    ...
                    {files.map((file, i) => (
                      <div key={i} className="flex items-center justify-between p-3">
                        <div className="flex items-center space-x-2.5 truncate">
                          ...
                          <span className="text-[10px] text-slate-500">{(file.size / 1024 / 1024).toFixed(2) !== 'NaN' ? `${(file.size / 1024 / 1024).toFixed(2)} MB` : 'Custom File'}</span>
                        </div>
                        <button 
              ...
              <div className="bg-dark-800/20 rounded-xl p-4 border border-dark-750">
                <p className="text-xs text-gray-400">
                  📋 <span className="font-semibold text-gray-300">Note:</span> Missing assets will be automatically synthesized by the AI text-to-3D pipeline.
                </p>
              </div>
            </div>
